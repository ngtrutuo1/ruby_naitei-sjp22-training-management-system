...
RSpec.describe Trainee::UserTasksController, type: :controller do
  ...
  let!(:user_subject)   { FactoryBot.create(:user_subject, course_subject: course_subject, user: user) }
  ...
  context "with a valid document" do
    it "attaches the document and sets success flash" do
      ...
      }.to change { user_task.reload.documents.count }.by(1)
      
      expect(user_task.documents.attached?).to be true
      expect(flash[:success]).to eq(I18n.t("trainee.user_tasks.update_document.document_updated"))
      expect(response).to redirect_to(trainee_course_subject_path(course_id: course.id, id: subject.id))
    end

    it "attaches the document with correct data" do
      patch :update_document, params: {
        id: user_task.id,
        task_id: task.id,
        user_subject_id: user_subject.id,
      ...
      expect(new_document.filename.to_s).to eq("sample_document_test.pdf")
      expect(new_document.content_type).to eq("application/pdf")
    end
  end
